<template>
  ...
  let shuffledQuotes = []; // 用于存储随机打乱后的语录列表
  let quoteIndex = 0; // 当前语录的索引
  
  // Fisher-Yates 洗牌算法，随机打乱语录数组
  const shuffleQuotes = () => {
  ...
  // 初始化加载时显示第一条语录
  const initQuote = () => {
    shuffleQuotes(); // 页面加载时先洗牌
    currentQuote.value = shuffledQuotes[quoteIndex];
    displayedText.value = currentQuote.value.sentence; // 直接显示第一句
  ...
  const getNextQuote = () => {
    ...
    if (quoteIndex >= shuffledQuotes.length) {
      quoteIndex = 0; // 所有语录显示完后重新开始
      shuffleQuotes(); // 重新打乱语录顺序
    }
  ...
  const showNextQuote = async () => {
    currentQuote.value = getNextQuote(); // 获取下一条随机语录
    ...
    setTimeout(showNextQuote, 100); // 延迟0.1秒再显示下一条
  };
  
  onMounted(() => {
    initQuote(); // 页面加载时，显示第一句
    setTimeout(showNextQuote, 3000); // 在3秒后开始循环随机显示语录
  });
  </script>
